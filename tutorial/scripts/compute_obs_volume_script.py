...
def main():
  ...
  for i in range(0, len(subjects)): 
    ...
    for j in range(0, len(seg_names)):
      ...
      cur_vol_file = '%sdecimated_%s_volume.txt' %(stats_dir, seg_names[j])
      volumes = []

      for k in range(0, len(subject_scans[i])):

        cur_shape = '%s%s/decimated_surfaces/%s.vtk' %(cur_dir, subject_scans[i][k], seg_names[j])
