...
def main():
  ...
  ####################################################################
  reg_path = "../regression/"
  output_base_path = "../reg_at_obs_time_pts/"
  ...
  seg_out_names = ['left_caudate', 'right_caudate', 'left_putamen', 'right_putamen']

  segs = ['Regression_baseline_init_baseline_left_caudate_trajectory___t_', 'Regression_baseline_init_baseline_right_caudate_trajectory___t_', 'Regression_baseline_init_baseline_left_putamen_trajectory___t_', 'Regression_baseline_init_baseline_right_putamen_trajectory___t_']
  ...
  for i in range(0, len(subjects)): 
    ...
    # but also intermediate, which we must calculatege
    ...
    for j in range(0, len(timepts_to_copy)):
      ...
      for k in range(0, len(segs)):
        ...
        data_in = "%s%s%d.vtk" %(cur_regression_dir, segs[k], timepts_to_copy[j])
        data_out = "%s%s_time_%0.2d.vtk" %(cur_output_dir, seg_out_names[k], j)
