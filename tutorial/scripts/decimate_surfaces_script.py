...
def main():
  ...
  if (not os.path.isfile(surfdecimate_path)):
    print "\nError: surfdecimate binary not found. Edit this script and ensure the path is correct.\n"
    sys.exit(1)
  ...
  for i in range(0, len(subjects)): 
    ...
    for j in range(0, len(subject_scans[i])):
      ...
      output_dir = data_path + subjects[i] + '/' + subject_scans[i][j] + '/decimated_surfaces/'
      if not (os.path.isdir(output_dir)):
        os.system('mkdir ' + output_dir)
      
      for k in range(0, len(seg_names)):
        
        cur_seg = data_path + subjects[i] + '/' + subject_scans[i][j] + '/surfaces/' + seg_names[k] + '.vtk'
        out_shape = output_dir + seg_names[k] + '.vtk'
        
        transform_cmd = surfdecimate_path + ' ' + cur_seg + ' ' + out_shape + ' 0.80'
